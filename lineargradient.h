...
public:
    ...
    static Eigen::Matrix<T, Eigen::Dynamic,1, _Options> solve(const Eigen::Matrix<T,Eigen::Dynamic, Eigen::Dynamic,_Options>& A,const Eigen::Matrix<T,Eigen::Dynamic, 1,_Options>& b,T epsilon=1e-6);
};
template<typename T, int _Options>
Eigen::Matrix<T, Eigen::Dynamic,1, _Options> LinearGradient<T,_Options>::solve(const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, _Options> &A, const Eigen::Matrix<T, Eigen::Dynamic,1, _Options> &b,T epsilon)
{
...
//    std::cout<<"d0:"<<dk<<std::endl;
    for(int i=1;i<=n;i++){
        if(gk.norm()<epsilon){
            ...
            alphak=gk.squaredNorm()/(dk.transpose()*A*dk);
            VectorXT xkplus(n,1);
            xkplus=xk+alphak*dk;
            VectorXT gkplus(n,1);
            gkplus=A*xkplus-b;
